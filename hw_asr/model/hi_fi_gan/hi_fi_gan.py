...
class Generator(nn.Module):
    def __init__(self, h_u, k_u, k_r, D_rs, l_relu_slope, **kwargs):
        ...
        self.blocks_seq.append(weight_norm(
        ...
        for l in range(k_u.shape[0]):
            ...
            self.blocks_seq.append(
                weight_norm(nn.ConvTranspose1d(cur_channels, cur_channels//2, kernel_size=ker_size,
        ...
        self.blocks_seq.append(
            weight_norm(nn.Conv1d(cur_channels, 1, 7, padding='same'))
        )
        self.forw_remove.append(len(self.blocks_seq) - 1)
        self.blocks_seq.append(
            nn.Tanh()
...
class Discriminator(nn.Module):
    ...
    def forward(self, audios, y_pred):
        y_ds_true, y_ds_pred, layer_true, layer_pred = self.msd(audios, y_pred)
        y_ds_true1, y_ds_pred1, layer_true1, layer_pred1 = self.mpd(audios, y_pred)
